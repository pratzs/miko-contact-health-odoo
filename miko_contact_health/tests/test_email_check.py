...
class TestEmailCheck(TransactionCase):
    ...
    def test_international_addresses_are_never_flagged(self):
        """RFC 6531 addresses are valid and common outside English speaking markets.

        Rejecting them would make every "muller", "jose" and CJK contact turn red
        for a German, French or Chinese customer, which is the single fastest way
        to get uninstalled. Only ASCII characters are policed; the alphabet is not.
        """
        for address in ('jose@example.com', u'jos\u00e9@example.com',
                        u'm\u00fcller@example.de', u'\u7528\u6237@example.cn',
                        u'test@m\u00fcnchen.de', 'test@xn--mnchen-3ya.de',
                        u'fran\u00e7ois.dupont@exemple.fr'):
            status, _detail = validate_email(address)
            self.assertEqual(status, 'ok',
                             '%r is a valid international address' % address)

    def test_length_limits_match_the_specification(self):
        self.assertEqual(validate_email('x' * 64 + '@example.com')[0], 'ok')
        self.assertEqual(validate_email('x' * 65 + '@example.com')[0], 'too_long')
...
class TestContactHealth(TransactionCase):
    ...
    def test_duplicates_are_found_across_recompute_chunks(self):
        """The rescan recomputes in chunks, so duplicate detection must not rely
        on both copies being in the same chunk. The query looks at the whole
        table, and this proves it by recomputing one of the pair alone.
        """
        a = self._partner('Chunk A', email='cross.chunk@example.com')
        b = self._partner('Chunk B', email='cross.chunk@example.com')
        # Recompute ONLY b: a is not in the recordset at all.
        b._compute_contact_health()
        self.assertEqual(b.ch_email_status, 'duplicate')

    def test_archived_contacts_do_not_create_phantom_duplicates(self):
        live = self._partner('Live one', email='reused@example.com')
        gone = self._partner('Archived one', email='reused@example.com')
        gone.active = False
        live._compute_contact_health()
        self.assertEqual(live.ch_email_status, 'ok',
                         'an archived contact must not make a live one a duplicate')
