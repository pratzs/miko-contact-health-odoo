...
def validate_email(address):
    ...
    # Only ASCII characters are judged. Accented and non-Latin local parts are
    # legitimate under RFC 6531 (internationalised email), and flagging every
    # "muller" or "jose" would make this module unusable outside English speaking
    # countries. Structure is still checked; the alphabet is not policed.
    bad_local = {c for c in local if c.isascii()} - LOCAL_ALLOWED
    if bad_local:
        return 'bad_chars', 'Illegal character before the @: %s' % ''.join(sorted(bad_local))
    if local.startswith('.') or local.endswith('.') or '..' in local:
        return 'bad_dots', 'Misplaced dot before the @'

    # Same for the domain: an internationalised domain name may be written in
    # unicode rather than punycode.
    bad_domain = {c for c in domain if c.isascii()} - DOMAIN_ALLOWED
    if bad_domain:
